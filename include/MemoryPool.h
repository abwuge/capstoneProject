...
public:
    inline static TF1 *getDetector_reconstructUsingLinearMethod_f1(const char *name, const char *formula, Double_t xmin, Double_t xmax);
...
inline TF1 *MemoryPool::getDetector_reconstructUsingLinearMethod_f1(const char *name, const char *formula, Double_t xmin, Double_t xmax)
{
...
{
    if (!Detector_particleHitData_particle)
        Detector_particleHitData_particle = new Particle();
...
inline TGraph *MemoryPool::getDetector_reconstructUsingLinearMethod_graph(Int_t n, const Double_t *x, const Double_t *y)
{
...
{
    if (!Detector_detect_detectedTimes)
        Detector_detect_detectedTimes = new std::vector<double>(size);
...
{
    if (!Detector_processReconstruction_hitTimes)
        Detector_processReconstruction_hitTimes = new std::vector<double>(size);
